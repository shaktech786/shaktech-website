...
const BlogPage = () => {
  ...
  const getTagColor = (tag: string) => {
    const colors = {
      "AI": "bg-primary-900/30 text-primary-300 border-primary-700",
      "Methodology": "bg-accent-900/30 text-accent-300 border-accent-700",
      "Leadership": "bg-creative-900/30 text-creative-300 border-creative-700",
  ...
  return (
    <main className="min-h-screen bg-primary-950">
      ...
      {posts.filter(post => post.featured).map((post) => (
        <section key={post.id} className="py-12 bg-primary-950">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Card className="bg-gradient-to-r from-primary-900 to-accent-900 border-accent-700 hover:shadow-2xl transition-all duration-300">
              <CardContent className="p-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
                  <div className="lg:col-span-2 space-y-4">
                    ...
                    <h2 className="text-3xl font-bold text-white group-hover:text-accent-400 transition-colors">
                      {post.title}
                    </h2>
      ...
      <section className="py-24 bg-primary-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.filter(post => !post.featured).map((post) => (
              <Card key={post.id} className="bg-gray-900 border-gray-700 hover:shadow-2xl hover:border-accent-500/50 transition-all duration-300 group">
                <CardHeader className="pb-4">
                  ...
                  <CardTitle className="text-xl text-white group-hover:text-accent-400 transition-colors line-clamp-2">
                    {post.title}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button variant="outline" className="w-full group mt-4 border-gray-600 text-gray-300 hover:border-accent-500 hover:text-accent-400" asChild>
                    <Link href={`/blog/${post.id}`}>
                      Read More
                      <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                    </Link>
